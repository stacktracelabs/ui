<template>
  <Breadcrumb>
    <BreadcrumbList>
      <BreadcrumbItem>
        <BreadcrumbLink as="a" :href="items[0].href">
          {{ firstLabel }}
        </BreadcrumbLink>
      </BreadcrumbItem>
      <BreadcrumbSeparator />
      ...
      <BreadcrumbItem v-for="item of remainingItems" :key="item.label">
        <template v-if="item.href">
          <BreadcrumbLink as="a" :href="item.href" class="max-w-20 truncate md:max-w-none">{{ item.label }}</BreadcrumbLink>
          <BreadcrumbSeparator />
        </template>
